...
int     DISPLAY_fluid_texture_mode = 3; //0;
...
public void setup() {
  ...
  opticalflow.param.display_mode = 2; //1;
  opticalflow.param.flow_scale = 8.0f;
  opticalflow.param.threshold = 0.0f;

  // fluid object
  fluid = new DwFluid2D(context, pg_movie_w, pg_movie_h, fluidgrid_scale);
  // initial fluid parameters
  fluid.param.dissipation_density     = 0.0f; //0.95f;
  fluid.param.dissipation_velocity    = 0.0f; //0.90f;
  fluid.param.dissipation_temperature = 0.0f; //0.70f;
  fluid.param.vorticity               = 0.0f; //0.30f;
  fluid.param.num_jacobi_projection   = 12;
  fluid.param.timestep                = 0.7f;
  fluid.param.gridscale               = 6.50f;
  ...
  movie = new Movie(this, "cat-look.mp4");
  movie.loop();
  movie.speed(0.1);
